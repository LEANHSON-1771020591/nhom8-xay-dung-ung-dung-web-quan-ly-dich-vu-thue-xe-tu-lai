...
<head>
    ...
    <title>Sửa Blog - Vato Admin</title>
...
<body class="bg-[#f5f7fb] overflow-x-hidden">
    ...
    <section class="ml-64 w-[calc(100vw-16rem)] max-w-[calc(100vw-16rem)] px-6 lg:px-8 py-10">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-4">Sửa bài Blog #{{ $blog->id }}</h1>
        <form method="POST" action="{{ url('/admin/blogs/'.$blog->id) }}" class="bg-white border border-gray-200 rounded-xl p-6">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="content_text_preview" class="w-full border rounded px-3 py-2" rows="3" required>{{ $blog->content_text_preview }}</textarea>
            </div>
            <div class="mb-4">
                <label class="block text-sm mb-1">Nội dung HTML</label>
                <textarea name="content_html" class="w-full border rounded px-3 py-2 font-mono" rows="10" required>{{ $blog->content_html }}</textarea>
            </div>
            <div class="flex gap-2">
                <button type="submit" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded">Cập nhật</button>
