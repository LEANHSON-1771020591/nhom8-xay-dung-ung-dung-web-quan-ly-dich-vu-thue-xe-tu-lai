...
<body>
    ...
    <div class="max-w-md mx-auto mt-10 bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <div class="flex flex-col items-center mb-6">
            ...
            <h1 class="mt-3 text-2xl font-bold text-gray-900">Đăng ký</h1>
        </div>
        <form method="POST" action="{{ url('/register') }}" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm text-gray-600 mb-1">Username</label>
                <input type="text" name="username" class="w-full border border-gray-300 rounded-lg px-3 py-2" required value="{{ old('username') }}">
            </div>
            <div>
